...
export function PhaseSelector(props: {
  ...
  const numPhases = getNumPhases();
  const phasesCompleted = getPhasesCompleted();

  function getNumPhases(): number {
    if (gameRooms.length === 0) return 0;
  ...
  function getPhasesCompleted(): number {
    if (gameRooms.length === 0) return 0;
      ...
      {numPhases > 0 &&
        Array.from({ length: numPhases }, (_, index) => (
          ...
          >
            ...
            >
              {phasesCompleted > index && <CheckCircleOutlined />}
            </div>
            <Typography
              style={{
